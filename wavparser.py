...
def parse(filepath):
	...
	wav_input = open(filepath, "rb")
	wav_bytes = bytearray(wav_input.read())
	...
	channel_data = [] # a list of lists, each sublist being a channel as a list of floats
	...
	for i in range(0, int(len(wav_data) / (byps * channel_num))):
		for k in range(0,channel_num):
			...
			channel_data[k].append(float(intnum / (2**(bit_depth-1) - 1)))

	return channel_data

def get_wav_bytes(channel_data, filepath=None, bitdepth=16, samplerate=44100):
	"""returns a wav file as a bytearray based on the inputted channel_data
	the channel_data must be a list of lists of floats"""
	if type(channel_data) != list:
		raise ValueError("The argument given was not a list")
	for lst in channel_data:
		if type(lst) != list:
	...
	for lst in channel_data:
		if len(lst) != len(channel_data[0]):
			raise ChannelException("Not all channels are of equal length.")

	total_samples = len(channel_data) * len(channel_data[0])
	...
	slicesize = int(len(channel_data) * bitdepth / 8)
	...
	start_of_file = start_of_file[:22] + len(channel_data).to_bytes(2,'little',signed=False) + samplerate.to_bytes(4,'little',signed=False) + int(samplerate * len(channel_data) * bitdepth / 8).to_bytes(4,'little',signed=False) + slicesize.to_bytes(2,'little',signed=False) + bitdepth.to_bytes(2,'little',signed=False) + start_of_file[36:]
	start_of_file += subchunk2size.to_bytes(4,'little',signed=False)
	wav_data = bytearray()
	for i in range(0, int(total_samples / len(channel_data))):
		for k in range(0, len(channel_data)):
			smpint = int(channel_data[k][i] * (2**(bitdepth-1) - 1))
			wav_data += (smpint.to_bytes(int(bitdepth/8),'little',signed=True))
	return start_of_file + wav_data

def save(channel_data, filepath=None, bitdepth=16, samplerate=44100):
	"""saves the bytes of a wav file to disk as an actual wav file"""
	wav_bytes = get_wav_bytes(channel_data, filepath, bitdepth, samplerate)
